...
int Game::delete_user(const string& username)
{
	...
	if(lookup != users.end()){					// found 
		if(lookup->second.status == User::ONLINE) return FAIL; 		//return fail if the user is online
		users.erase(lookup);      //delete by iterator.
		return SUCCESS;
		}
	else{			// not found
...
int Game::logout_user(const string& username)
{
	auto user = users.find(username);
	if(user != users.end()){
		//auto matched = matches.find(user->match_id);
		//if( matched != matches.end()) return USER_IN_MATCH;
		if(users[username].match_id != 0) return USER_IN_MATCH;
...
unsigned long Game::make_match(const string& party_id)
{
    ...
    auto party = parties.find(party_id);
	if(party == parties.end()) return NULL_MATCH_ID;
	for( const string& username : party->second.members){
		if(users[username].match_id != 0)
			return NULL_MATCH_ID;	
       }
       
    Match newMatch = Match();
	newMatch.status = Match::IN_GAME;
	matches.emplace(next_match_id,newMatch);
	int result = add_party_to_match( party_id, next_match_id);
     if(result == SUCCESS) return next_match_id;
     return NULL_MATCH_ID;
      /*
     Match ma = Match();
     ma.status = Match:: IN_GAME;
     matches.emplace(next_match_id, ma);
     for(const string & username:party->second.members){
...
unsigned long Game::find_match(const string& party_id, long tol)
{
   auto party = parties.find(party_id);
	...
	long sumParty=0;
	long sumMatch=0;
	for(auto & value: party->second.members){
		sumParty+=users[value].us.experience;
	}
	long ave_party = sumParty/(party->second.members.size());
	
	
	for(auto it = matches.begin(); it != matches.end(); ++it){
		int totalnum = it->second.team1.size()+it->second.team2.size();
		for(auto & team1Mem: it->second.team1) sumMatch+=users[team1Mem].us.experience;
		for(auto & team2Mem: it->second.team2) sumMatch+=users[team2Mem].us.experience;
		long ave_match = sumMatch/totalnum;
		if(abs(ave_match-ave_party) <=tol)
			return it->first;
	}
...
{
   /* std::string new_party_id;
	for(std::vector<string>::iterator it = usernames.begin(); it!=usernames.end(); ++it){
		new_party_id.append(*it);
	}
	Party new_par = new Party();
	new_par.members = usernames;
	parties[new_party_id] = new_par;
	*/
    /* Your code goes here! */
   // return new_party_id;
   return "";
...
{
    auto user_name = users.find(username);
	if(user_name != users.end()){
		us.wins = users[username].us.wins;
		us.losses = users[username].us.losses;
		us.win_streak = users[username].us.win_streak;
		us.playtime = users[username].us.playtime;
		us.experience = users[username].us.experience;
		return SUCCESS;
