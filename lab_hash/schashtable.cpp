...
void SCHashTable<K,V>::remove( K const & key ) {
  ...
  //  if(indx >= size || indx < 0) return;
    for(it = table[indx].begin(); it!=table[indx].end(); it++){
	if(it->first == key){
		table[indx].erase(it);
		break;
	}
	...
	elems = elems-1;
//    table[indx].erase(table[indx].begin(),table[indx].end());
    /**
...
void SCHashTable<K,V>::resizeTable() {
	...
	list<pair<K,V> >* temp = new list<pair<K,V> >[size];
	for(int i = 0; i< oldSize; i++){
		for(it = table[i].begin(); it!=table[i].end(); it++){
			int fncKey =hash( it->first, size);
			temp[fncKey].push_front(*it);
		}
}	
    delete [] table;
    table = temp;
