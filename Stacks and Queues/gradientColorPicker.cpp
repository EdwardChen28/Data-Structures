...
{
	RGBAPixel color;
	int d =abs(ctrX-x)+abs(ctrY-y);
	if( d > radi)
	color = color2;
	else{
	color.red = color1.red - floor(d*(color1.red)/radi) + floor(d*(color2.red)/radi);
	
	color.green = color1.green - floor(d*(color1.green)/radi) + floor(d*(color2.green)/radi);
	
	color.blue = color1.blue - floor(d*(color1.blue)/radi) + floor(d*(color2.blue)/radi);
	}
